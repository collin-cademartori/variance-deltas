...
import * as d3 from "d3";
    ...
    d3.select("#highlight_container").selectAll(".highlight_box")
        ...
        .join(
          (enter) => {
            ...
            g.transition().duration(transition_dur/ 4).attr("opacity", 1);
            return(g);
          },
          (update) => update,
          (exit) => exit.transition().duration(transition_dur / 4).attr("opacity", 0).remove()
        );
  ...
  export function draw_geometry(
    ...
    tree_elem.selectAll(".tree_branch")
             ...
             .join((enter) => {
                ...
                if(!draw_static) {
                  g.transition().duration(transition_dur).attr("opacity", 1);
                }
            ...
            (update) => {
              update.select(".branch_path").transition().duration(transition_dur)
                     .attr("d", (d) => link([d.parent, d.child]))
              update.select(".select_path").transition().duration(transition_dur)
                     .attr("d", (d) => link([d.parent, d.child]))
              return update;
            },
            (exit) => {
              exit.transition().duration(transition_dur).attr("opacity", 0).remove();
            }
          );
    ...
    tree_nodes.join((enter) => {
      ...
      update.select(".node_rect").transition().duration(transition_dur)
  ...
  export function draw_tree(
      ...
      selection?.on_change(() => {
        for(const channel of selection_channels) {
          ...
          draw_geometry(
            ...
            sel_colors[channel], sel_colors[channel], false, "_" + channel, 1.15, transition_dur / 4
          );
        }
    ...
    hover.on_change(() => {
      ...
      draw_highlight(ft, x, l_height, transition_dur);
    });
    ...
    const max_y = y(tree.map((fn) => fn.x_pos).reduce((p,n) => Math.max(p,n)) - 1) + 85;
    ...
    draw_geometry(
      tree, "tree_g", x, y, ss, draw_color, draw_color, draw_static, id_mod, 1, transition_dur
    );
    ...
    (update) => {
      update.select(".label_fo").transition().duration(transition_dur)
        .attr("x", (d : flat_node) => x(0.003 + d.ered))
        .attr("y", (d: flat_node) => y(d.label_y ?? 0));
